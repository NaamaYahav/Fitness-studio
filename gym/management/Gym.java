...
public class Gym {
    ...
    public String toString(){
        ...
        employeesData+=s1;
            String sessionsData="\nSessions Data:\n";
        for (Session session:getSecretary().getAllSessions()) {
            String info = "Session Type: " + session.getSessionType() + " | Date: " + session.getDate() + " | Forum: " + session.getF() + " | Instructor: " +
                    session.getInstructor().getName() + " | Participants: " + session.getParticipants().size() + "/" + session.getNumOfParticipants()+"\n";
            sessionsData+= info;
        }
